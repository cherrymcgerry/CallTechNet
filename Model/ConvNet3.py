...
class cnn(nn.Module):
    def __init__(self, nc, nf, extra_layers, use_sigmoid=True):
        ...
        submodule = DownConv(nf*f_mult,101, final=True, submodule=submodule)
...
class DownConv(nn.Module):
    def __init__(self,input_nc, output_nc, kernel=4, final=False, submodule=None):
        ...
        down_conv = nn.Conv2d(input_nc, output_nc, kernel_size=kernel, stride=2, padding=1, bias=False)
        down_norm = nn.BatchNorm2d(output_nc)
        down_relu = nn.LeakyReLU(0.01, True)
